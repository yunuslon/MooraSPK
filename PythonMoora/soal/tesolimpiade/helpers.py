...
def ListTesOlimpiade(tp):
    if len(tp)>0:
        cols = ['mata_pelajaran','no','gambar','pertayaan',
                'jawabanA','jawabanB','jawabanC','jawabanD','jawabanE','kunci','id']
...
def SleksiSoalBio(tp):
    ...
    tmpA=list(zip(c.mata_pelajaran,c.no,c.gambar,c.pertayaan,
                  c.jawabanA,c.jawabanB,c.jawabanC,c.jawabanD,c.jawabanE,c.kunci,c.id))
    def soal_id():
        tmp=[]
        for i in range(len(c)):
            if tmpA[i][0] == "biologi":
                tmp.append(tmpA[i][10])
    ...
    gabung= pd.concat([soal_id(),no(),gambar(),pertayaan(),jawabanA(),jawabanB(),jawabanC(),jawabanD(),jawabanE(),kunci()],
                      axis=1)

    return gabung

def SleksiSoalFis(tp):
    c= ListTesOlimpiade(tp)
    tmpA=list(zip(c.mata_pelajaran,c.no,c.gambar,c.pertayaan,
                  c.jawabanA,c.jawabanB,c.jawabanC,c.jawabanD,c.jawabanE,c.kunci,c.id))
    def soal_id():
